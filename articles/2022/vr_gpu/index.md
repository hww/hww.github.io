...
![Draw Calls Cost](images/image6.png)
*Влияние вызовов отрисовки (draw calls) на стоимость рендеринга. Элементы (SM + SMesh + SMesh) — главный приоритет для 3D-моделлера.*
...
![Chess Board](images/image16.jpg)
*Шахматные фигуры требуют всего двух материалов — черного и белого.*
...
![Texture Atlas with Colors](images/image9.png)
*Текстурный атлас с чистыми цветами.*

![Texture Atlas with subtextures](images/image15.png)
*Текстурный атлас с субтекстурами (изображениями).*
...
![1-2 Materials World](images/image3.jpg)
*Такая сцена может быть отрисована небольшим количеством текстур и материалов.*
...
![Large overlapped objects](images/image17.jpg)
*Крупные пересекающиеся объекты не дружественны для рендеринга на TBR-платформах.*
...
![Sweet Spot](images/image19.png)
*У каждой платформы есть своя оптимальная зона производительности (sweet spot) для отрисовки множества объектов с различным размером мешей. Например, для Oculus Quest наилучшая производительность достигается при использовании объектов в диапазоне 5–7 тысяч треугольников на один батч.*
...
![Texture size threshold](images/image14.png)
*У платформы возникают проблемы, когда размер текстуры превышает определённый порог.*
...
![Normal defects](images/image8.png)
*Дефекты, вызванные некорректными нормалями.*

### UV-развертка

Всегда проверяйте UV-развертку с помощью **шахматной текстуры** (checkerboard). Это позволяет выявить растяжения и искажения.

![Checkerboard comparison](images/image7.png)
*Слева — хорошая развертка, справа — плохая (растянутые UV).*
