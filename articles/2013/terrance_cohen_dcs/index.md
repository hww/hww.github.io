...
* Используются две иерархии типов компонентов, для разделения состояния и действия. Одно дерево описывает что происходит, а другое — как это происходит:
    ...
    2. **`ShotAction`**: Представляет конкретное состояние выстрела в данный момент (например, движение вперед, полет по дуге).
       ...
       * Shared — это оптимизация памяти:
         1. У всех пуль может быть общее действие "Лететь вперёд".
...
> **⨠ Примечания автора**
> Shared `ShootAction` самый сомнительный пункт в списке. Это буквально антипаттерн DOP. Единственное рациональное объяснение этому может быть ограничена память PlayStation 3.
