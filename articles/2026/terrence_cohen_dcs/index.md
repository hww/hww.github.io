...
* **Ростер (Roster):** Это массив индексов, который хранит ссылки на все *выделенные* экземпляторы компонентов в пуле.
...
| Параметр | Тип | Смысл |
| -- | -- | -- |
| name | const char* | Имя типа. Это просто строка для отладки и для Lua (чтобы скрипты могли обращаться по имени, а не по числу). В рантайме она не используется для логики, только для удобства. |
...
* `LocationEvent` будет обновляться на SPU во время `PreUpdate` для опроса условий (например, проверки позиции), а затем на PPU во время `PostUpdate` для вызова колбэков, если условие выполнено.
* Для повышения производительности, опрос условий можно вынести полностью на SPU. Если условие выполнено, SPU-задача может добавить компонент в общий список для вызова колбэков на PPU, что минимизирует нагрузку на главное ядро.
...
* Используются две иерархии типов компонентов:
    1. **`Shot`**: Представляет конечный автомат состояния выстрела. **Не обновляется** автоматически, а уведомляется о событиях от текущих действий.
    2. **`ShotAction`**: Представляет конкретное состояние выстрела (например, движение вперед, полет по дуге). Эти компоненты разделяются между разными типами `Shot` и **автоматически и асинхронно обновляются** системой.
