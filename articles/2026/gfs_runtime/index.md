...
## Реализация компактного дерева: Структура из трех указателей
...
## 2.3. Битовые маски процессов: Многократный обход и контекстная фильтрация
...
## Исторический контекст: Низкоуровневые маски в OpenGOAL
...
## 4.1. Анатомия контейнера данных: Полиморфизм, Типизация и Object Pooling
...
## Реализация конкретного сообщения: Контекст управления камерой
...
## 4.2. Механика диспетчеризации: Атомарность переходов и Топологическое вещание
...
## Глубокий разбор системной логики
...
## Раздел 5. Топология очередей: Три стратегии доставки сообщений
...
## GUI-контроллер как эталон применения локальной очереди
...
## Глава 3. API жизненного цикла процесса и каскадное управление
...
## 3.1. Активация процесса (Activate)
...
## Архитектурные нюансы фазы активации
...
## 3.2. Деактивация процесса (Deactivate)
...
## Архитектурные нюансы фазы деактивации
...
## 3.3. Разрыв логических связей (Disconnect)
...
## Архитектурные нюансы фазы разрыва связей
...
## Сводная матрица состояний жизненного цикла
...
## 📝 **Раздел: API перехода в другое состояние**

---

### **3.8. API перехода между состояниями (`GoHook`)**
...
### **3.8.2. Режимы приоритета перехода**
...
### **3.8.3. Микро-нюансы перехода (связь с разделами 3 и 4)**

#### **A. Вызов из `Code` (основной корутины)**
...
#### **B. Вызов из побочных методов (`Post`, `Trans`, `Exit`, `Enter`, `OnEvent`)**
...
### **3.8.4. Защита от рекурсии (State Loop Guard)**
...
### **3.8.5. Передача параметров перехода (`transitionParams`)**
...
### **3.8.6. Связь с Handle-системой и Message Queue**
...
### **3.8.7. Полный пример использования**
...
### **3.8.8. Таблица: Где какой приоритет использовать**
...
### **3.8.9. Связь с кодом `kernel-dispatcher`**
...
## 3.4. API перехода между состояниями (GoHook)

Физическое переключение состояний (State Transition) в детерминированном рантайме — это не просто смена ссылки на текущий стейт. Это контролируемая хирургическая операция над контекстом исполнения процесса, которая должна гарантировать атомарность, отсутствие «кадров-призраков» и полную безопасность для памяти.
В нашей архитектуре этот процесс инкапсулирован в метод GoHook():

```cs
/// <summary>
/// Запрашивает переход в новое логическое состояние с передачей параметров.
/// </summary>
/// <param name="nextState">Целевое состояние, в которое переходит процесс.</param>
/// <param name="transitionParams">Структурный контейнер с аргументами для инициализации нового состояния.</param>
/// <param name="transitionPriority">Приоритет: немедленное переключение контекста или отложенная смена.</param>
public void GoHook(
    State nextState, 
    in Message transitionParams = default, 
    TransitionPriority transitionPriority = TransitionPriority.Delayed)
{
    // Системная реализация переключения контекстов
}
```

------------------------------

## 3.4.1. Режимы приоритета переходов

Чтобы рантайм оставался предсказуемым, мы обязаны строго разграничивать, откуда именно был запрошен переход. Для этого вводится перечисление TransitionPriority:

```cs
public enum TransitionPriority
{
    /// <summary>
    /// Переход происходит мгновенно внутри текущего контекста исполнения.
    /// Допустим только при вызове из основного потока/корутины (Code).
    /// </summary>
    Immediate,

    /// <summary>
    /// Переход кэшируется и выполняется в ближайшей безопасной точке диспетчера.
    /// Используется при вызовах из вспомогательных методов (Post, Trans, OnEvent).
    /// </summary>
    Delayed
}
```

## Сценарий А: Мгновенный переход (Immediate) из главного потока Code

Когда геймплейная логика внутри основной корутины Code() понимает, что условие выполнено (например, здоровье врага упало до нуля), она вызывает GoHook(DeathState, ..., TransitionPriority.Immediate).

* Рантайм-эффект: Переход выполняется незамедлительно в текущем кадре. Любой код, написанный в текущей корутине ниже инструкции GoHook(), полностью обрывается и никогда не будет выполнен.
* Жизненный цикл: Ядро мгновенно вызывает делегат Exit() старого состояния, затем Enter() нового состояния, передавая туда параметры transitionParams, и сразу же запускает новую корутину Code() до её первого прерывания (suspend).

## Сценарий Б: Отложенный переход (Delayed) из побочных методов

Если логика смены стейта срабатывает внутри реактивных методов (например, в Post(), выполняющемся каждый кадр, или в OnEvent() при получении сообщения о попадании снаряда), мгновенно разрушать контекст нельзя. В оригинальном коде OpenGOAL от Naughty Dog такие методы, как trans-hook и post-hook, физически выполнялись во временных процессорных потоках (cpu-thread), из-за чего прямой сброс стека привел бы к крашу.

* Рантайм-эффект: Физический переход не происходит мгновенно. Целевое состояние просто сохраняется во внутреннюю системную переменную _pendingState, а текущий метод (Post или OnEvent) штатно дорабатывает до конца своей закрывающей скобки.
* Жизненный цикл: Физическая процедура Exit() → Enter() выполнится на следующей итерации диспетчера.

------------------------------

## 3.4.2. Защита от бесконечной рекурсии (State Loop Guard)

Самая опасная уязвимость «мгновенных» переходов — это риск возникновения бесконечной логической петли, когда разработчик случайно вызывает Immediate-переход прямо из конструктора или метода инициализации состояния:
Enter(State1) → GoHook(State2) → Enter(State2) → GoHook(State1) → ...
В классических движках это приводит к мгновенному падению потока по ошибке StackOverflowException. В нашем детерминированном рантайме предусмотрен жесткий предохранитель:

```cs
private void PerformImmediateTransition(State nextState, in Message transitionParams)
{
    // Защитный флаг: если переход уже выполняется в данном потоке
    if (_isTransitioning)
    {
        // Аварийный перехват: переводим процесс в специальный статус-предохранитель
        _status = ProcessStatus.InitializeGo; 
        return;
    }
    ...
    try
    {
        Exit();
        Enter(nextState, transitionParams);
        Code(); // Выполнение основного тела до первого suspend/yield
    }
    finally
    {
        // Сброс флага блокировки при безопасном выходе
        _isTransitioning = false;
    }
}
```

Благодаря статусу ProcessStatus.InitializeGo, планировщик перехватывает зацикливание, принудительно останавливает лавину вызовов и позволяет корректно закрыть текущий стейт до того, как стек процессора будет переполнен.

------------------------------

## 3.4.3. Память и передача параметров перехода

Для передачи аргументов в новое состояние (например, координат точки назначения при переходе ИИ в режим погони) метод GoHook() принимает ссылку на структуру in Message.
Использование модификатора in гарантирует, что структура передается исключительно по ссылке, предотвращая скрытое копирование 48 байт данных в стеке. Внутри метода Enter() нового состояния аргументы распаковываются без создания мусора в куче (Zero GC) благодаря нашему низкоуровневому выравниванию StructLayout:
...
public void Enter(State state, in Message transitionParams)
{
    ...
    {
        case MessageType.MsgVec3:
            // Мгновенное чтение без аллокаций благодаря перекрытию памяти (Union)
            Vector3 targetPos = transitionParams.vec3; 
            InitializeChaseBehavior(targetPos);
            break;

        case MessageType.MsgIVec3:
            IntVector3 gridCoordinates = transitionParams.ivec3;
            InitializeGridNavigation(gridCoordinates);
            break;
    }
}
```
...
## 3.4.4. Матрица применения приоритетов GoHook

Чтобы не нарушать детерминизм рантайма, разработчик должен строго следовать архитектурной матрице вызовов:

| Точка вызова GoHook | Рекомендуемый приоритет | Техническое обоснование |
| --- | --- | --- |
| Code() (Основная корутина) | Immediate | Контекст выполнения стабилен. Можно безопасно прервать текущий кадр и запустить новый стейт. |
| Post() (Пост-обработка кадра) | Delayed | Метод работает во временном контексте. Смена стейта обязана быть отложена до следующей итерации диспетчера. |
| Trans() (Условия переходов) | Delayed | Аналог OpenGOAL trans-hook. Должен доработать до конца фазы анализа условий. |
| Enter() (Вход в состояние) | Delayed (С защитой) | Высокий риск бесконечной рекурсии. Требует активации State Loop Guard. |
| Exit() (Выход из состояния) | Delayed | Запрещено менять целевой стейт, пока рантайм выполняет очистку ресурсов предыдущего. |
| OnEvent() (Обработка сообщений) | Delayed | Обработка текущего пакета/сообщения в очереди кадра не должна прерываться на середине. |

------------------------------

## 3.4.5. Архитектурная преемственность с ядром OpenGOAL

Этот дуализм переключений контекста напрямую наследует логику распределения задач в коде kernel-dispatcher движков Naughty Dog. В Лисп-среде GOAL немедленный переход внутри основного потока (resume-hook) осуществлялся макросом go:

```lisp
;; Внутри корутины (Code) -> Мгновенный Immediate-переход
(go 'chase-state) 
```

Если же требовалось сменить состояние из других фаз (trans-hook или post-hook), программист был обязан напрямую выставить переменную next-state, что полностью эквивалентно нашему приоритету Delayed:

```lisp
;; Внутри побочных методов -> Отложенный Delayed-переход
(set! (-> this next-state) 'chase-state) 
```

Затем на следующей итерации диспетчер ядра проверял этот регистр, атомарно закрывал старый стейт через exit и инициализировал новый через enter:
...
## 3.7. Двухфазная инициализация и механизмы отложенного планирования
...
## 3.7.1. Дуализм инициализации: InitFromEntity против InitByOther
...
## 3.7.2. Отложенный запуск действий через статусы планировщика
...
## Техническая суть и статусная модель отложенного запуска
...
## Раздел 6. Концепция прокси-компонентов: Интеграция процессов с визуальными данными сцены
...
## Вариант А: Прокси как плоский контейнер фиксированных ссылок (Данные)
...
## Вариант Б: Прокси как унифицированный интерфейс доступа (Код)
...
## Раздел 7. Концепция Entity: Активные прокси и детерминированное «воскрешение» объектов сцены
