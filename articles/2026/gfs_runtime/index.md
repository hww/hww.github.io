...
## 7.13. Сводка: Два измерения жизненного цикла
...
│     OnDisable() → Kill()               │ ──► Deactivate() → Dead
└──────────────────┬─────────────────────┘
                   │ (Сброс несериализуемых полей в null)
                   ▼
┌────────────────────────────────────────┐
│      ФАЗА ВКЛЮЧЕНИЯ (Post-Rebuild)     │
│     OnEnable() → Birth()               │ ──► RunInitAtNextFrame() → Init
└────────────────────────────────────────┘
...
## Глава 9. Концепция Entity: Активные прокси и детерминированное «воскрешение» объектов сцены
...
## 10. Заключение: Детерминизм как архитектурный выбор
...
│                      В РЕДАКТОРЕ (Unity)                    │
│  ┌─────────────────────────────────────────────────────────┐│
│  │  [Префаб] → Сложная структура компонентов, материалов  ││
│  └────────────────────────┬────────────────────────────────┘│
│                           │                                 │
│                           ▼                                 │
│  ┌─────────────────────────────────────────────────────────┐│
│  │           ПРОКСИ-КОМПОНЕНТ (Тонкая обёртка)             ││
│  │    • Фиксированные имена (RightHandBone)                ││
│  │    • Интерфейсы доступа (IRendererProxy)                ││
│  └────────────────────────┬────────────────────────────────┘│
└───────────────────────────┼─────────────────────────────────┘
                            │
                            ▼
┌─────────────────────────────────────────────────────────────┐
│                   ИЗОЛИРОВАННЫЙ РАНТАЙМ                     │
│  ┌─────────────────────────────────────────────────────────┐│
│  │              ПРОЦЕССЫ (Чистый C# код)                   ││
│  │  • Игровая логика (AI, физика, управление)              ││
│  │  • Стейт-машины (FSM)                                   ││
...
### 10.4. Сквозной жизненный цикл: от Entity до состояния
